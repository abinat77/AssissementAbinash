...
public class LoginPage extends BaseTestPage implements BaseTestPageFactory{ 
	...
	public LoginPage userLogIn() {
		try {
			String uName = getName();//Fetching login username from TestData.xls
			String pwd = getPwd(); //Fetching login password from TestData.xls
			signIn(uName, pwd); //Calling login method
